...
1. Commit only this task if needed; open a pull request (or the repo’s equivalent) with a one-line purpose; **leave it unmerged** unless the user asked to merge.
2. Detect whether a pipeline exists (`gh`, the host’s CI files, or the repo docs).
3. Merge only on a real green signal AND a merge license, using the repo’s normal method.
...
7. If you cannot commit or open a PR, set `needs_user` with why — not a skip.
